...
-- Write a query to print last name email and job title for each employee
...
SELECT d.department_id, e.first_name, e.last_name, e.job_id, d.department_name from departments d JOIN employees e
ON e.department_id = d.department_id
...
--8. Write a query to display the employee number, name (first name and last name), and salary for all employees who earn more than the average salary and who workin a department with any employee with a J in their name.
...
SELECT e.first_name, e.last_name, d.department_id, l.city, l.state_province from employees e JOIN departments d
ON e.department_id = d.department_id
JOIN locations l ON d.location_id = l.location_id
...
SELECT first_name, last_name, salary, department-id from employees
WHERE salary = (SELECT MIN(salary) FROM employees GROUP BY department_id ORDER BY MIN(salary) ASC);
...
SELECT e.first_name, e.last_name, e.employee_id, j.job_title, l.city FROM employees e JOIN jobs j
ON e.job_id = j.job_id 
JOIN departments d  ON e.department_id = d.department_id
JOIN locations l ON d.location_id = l.location_id
WHERE city = 'Toronto';
...
SELECT d.department_name, AVG(salary) AS average_salary from departments d JOIN employees e 
ON e.department_id = d.department_id
GROUP BY d.department_name
HAVING AVG(salary) > 7000;
...
--41. Write a query which is looking for the names of all employees whose salary is greater than 50% of their department’s total salary bill.
 ...
 ON e.department_id = d.department_id
 WHERE salary > (SELECT SUM(salary) from employees * 0.5);
...
SELECT d.department_name, l.postal_code from departments d JOIN locations l
ON d.location_id = l.location_id
WHERE city = 'London';
